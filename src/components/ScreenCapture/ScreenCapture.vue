<template>
  <div id="screens-list">
    ...
    <div v-for="item in list" :key="item.sourceId" :label="item.sourceName" class="screen-info"
      ...
      <p>
        <el-button type="text">
          {{item.sourceName.length > 20 ? item.sourceName.slice(0,20)+'...' : item.sourceName }}
        </el-button>
      </p>
    </div>
  </div>
...
const logger = new Log('ShowScreenCapture');
export default {
  data() {
    return {
    };
  ...
  methods: {
    ...
    renderScreensList() {
      ...
      for (let i = 0; i < list.length; i += 1) {
        srcInfos = list[i];
        if (srcInfos.thumbBGRA.length !== 0) {
          ...
          imgData = new ImageData(
            new Uint8ClampedArray(srcInfos.thumbBGRA.buffer),
            srcInfos.thumbBGRA.width,
            srcInfos.thumbBGRA.height,
          );
